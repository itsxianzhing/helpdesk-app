...
import type { TicketDetailResponse, TicketPriority, TicketStatus } from '../../tickets/types';
import { ApiError } from '../../../lib/apiError';
import { getTicketById, updateAdminTicket } from '../../tickets/api/ticketApi';
...
function AdminTicketDetail({ ticketId }: AdminTicketDetailProps) {
  const { auth } = useAuth();
  ...
  async function handleUpdate() {
    ...
    try {
      const response = await updateAdminTicket(ticket.id, {
        status,
        priority,
      ...
      if (error instanceof ApiError) {
        if (error.statusCode === 409) {
          setActionError('This ticket was modified by another user. Please refresh and try again.');
        } else {
  ...
  return (
    <div className="overflow-hidden rounded-lg border bg-background">
      ...
      <div className="border-b p-4 sm:p-6">
        ...
        <h1 className="mt-1 break-words text-2xl font-semibold tracking-tight">{ticket.title}</h1>

        <p className="mt-2 text-sm text-muted-foreground">Submitted by {ticket.userName}</p>
      </div>
